...
double operator+(const Power &rhs, const Power &lhs) noexcept
{
    ...
    return std::sqrt((rhs.power_active + lhs.power_active) * 2 +
...
int power_main_example()
{
    ...
    spdlog::info("----- Power Example -----");

    // launch summation of 2 vectors one by one vec1[i] + vec2[i] in parallel
    std::vector<Power> vec1 = {Power(1, 2),
                               Power(3, 4),
                               Power(5),
                               Power(1, 2),
                               Power(3, 4),
                               Power(5)};
    std::vector<Power> vec2 = {Power(10, 20),
                               Power(30, 40),
                               Power(50),
                               Power(10, 20),
                               Power(30, 40),
    ...
    for (size_t i = 0; i < vec1.size(); i++)
    {
        threads.emplace_back(
            [&mtx, &results](const Power &p1, const Power &p2) {
